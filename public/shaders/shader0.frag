...
float march(vec3 ro, vec3 rd, float rmPrec, float maxd, float mapPrec)
{
    ...
    for(int i=0;i<250;i++)
    {
        if (s<0.25*log(d*d/s/500.)||s>maxd) break;
        s = map(ro+rd*d).x;
        d += s*0.2;
    }
    return d;
...
/*    if ( iMouse.z>0.) cam_e = iMouse.x/iResolution.x * 10.; // mouse x axis
	...
	vec2 uv = gl_FragCoord.xy / vec2(Width, Height) * 2. -1.;
    uv.x*=Width/Height;
    ...
    if (d<maxd)
    {
        ...
        vec3 cubeRefl = vec3(1.0);//textureCube(iChannel0, reflRay).rgb * refl_i;
        vec3 cubeRefr = vec3(1.0);//textureCube(iChannel0, refrRay).rgb * refr_i;

        col = cubeRefl + cubeRefr + pow(b, 15.);

       	// lighting from iq shader (mike)
    ...
    {
        b+=0.1;
        col = vec3(1.0, 0, 0);//textureCube(iChannel0, rd).rgb;
    }
